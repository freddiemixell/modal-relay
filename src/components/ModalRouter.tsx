...
export const ModalRouter = ({ children, modalRoot }: ModalRouterProps) => {
  let modals = useModalStore(selector);
  let filteredModals = React.Children
    .toArray(children)
    .filter((child) => React.isValidElement(child) && modals.includes(child.props?.id));

  // Destructure the first modal so we only ever show one at a time.
  const [firstModal = null] = filteredModals;

  return modalRoot
    ? ReactDOM.createPortal(<React.Fragment>{firstModal}</React.Fragment>, modalRoot)
    : null;
};
